...
const FriendPostSelected = () => {
    ...
    let {title, postPic, postTime, description, username, profilePic} = location.state;
    return (
        <>
            <div className='flex flex-col items-start px-8 pt-8 pb-36'>
                ...
                <div className='py-2 px-4  bg-gray-200 text-gray-400 w-full rounded-lg text-sm'>Add a comment...</div>
                <Comment username={username} profilePic={profilePic} content={"wow this is great! really feeling the kachow with this one!"} timePosted={"8 hr ago"}/>
                <Comment username={username} profilePic={profilePic} content={"mmmm, not a fan. do better next time"} timePosted={"1 hr ago"}/>
                <Comment username={username} profilePic={profilePic} content={"I CHANGED MY MIND I LOVE IT!!!!"} timePosted={"3 min ago"}/>
            </div>
